...
def main():
    ...
    with open(out_asm, "w", encoding="utf-8") as fh:
        fh.write(";; Generated by tools/mkpack.py from %s - do not edit.\n;;\n"
                 % name)
        fh.write(";; The game's tables, LZSS packed. See the tool for the\n")
        fh.write(";; format; src/unpack.asm has the decoder, forty bytes of it.\n")
        fh.write(";; %d bytes of tables in %d, %d literals and %d matches.\n\n"
                 % (len(raw), len(packed), lits, matches))
        fh.write("TABLE_RAW_LEN    EQU %d\n" % len(raw))
        fh.write("TABLE_PACKED_LEN EQU %d\n\n" % len(packed))
        fh.write("table_packed\n")
